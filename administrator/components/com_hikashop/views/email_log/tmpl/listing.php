...
<form action="<?php echo hikashop_completeLink('email_log'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php } ?>
	<table id="hikashop_email_log_listing" class="adminlist table table-striped table-hover" cellpadding="1">
		<thead>
			<tr>
...
?>
			<tr class="row<?php echo $k; ?>">
				...
				<td>
					<?php if($this->manage){ ?>
						<a href="<?php echo hikashop_completeLink('email_log&task=edit&cid[]='.(int)$row->email_log_id); ?>">
					<?php } ?>
							<?php echo $row->email_log_subject; ?>
					<?php if($this->manage){ ?>
						</a>
					<?php } ?>
				</td>
