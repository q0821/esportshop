...
if($version[0] < 5) {
	echo '<html><body><h1>This extension works with PHP 5 or newer.</h1>'.
		...
		'installation abort...</body></html>';
	exit;
} else if(version_compare(JVERSION, '2.5.0', '<') && version_compare($hikashopConfig->get('version', '1.0'), '2.4.0', '<')) {
	echo '<html><body><h1>This extension works with HikaShop 2.0 or newer.</h1>'.
	...
	class com_hikaserialInstallerScript {
		...
		function preflight($type, $parent) {
			...
			if(version_compare($hikashopConfig->get('version', '1.0'), '2.4.0', '<')) {
				echo '<h1>This extension works with HikaShop 2.4.0 or newer.</h1>'.
					'<h2>Please install the latest version of HikaShop before.</h2>'.
					'installation abort.';
				if($type == 'update')
					JError::raiseWarning(null, 'Cannot update HikaSerial 1.9.1 without HikaShop 2.4.0 or newer');
				else
					JError::raiseWarning(null, 'Cannot install HikaSerial 1.9.1 without HikaShop 2.4.0 or newer');

				$joomConf = JFactory::getConfig();
				$debug = $joomConf->get('debug');
				if($debug) {
					JError::raiseError(500, 'Cannot install HikaSerial 1.9.1 without HikaShop 2.4.0 or newer');
				}
				return false;
			}
			return true;
