...
?><?php
class hikaserialPluginsClass extends hikaserialClass {
	...
	public function getTable() {
		if(!HIKASHOP_J16) {
			return hikaserial::table('plugins', false);
		}
		return hikaserial::table('extensions', false);
	}
	...
	public function getPlugin($name, $type = 'generator') {
		$ret = $this->getPlugin($type,$name);
		if(!empty($ret)){
			if(count($ret) == 1)
				return reset($ret);
			return $ret;
