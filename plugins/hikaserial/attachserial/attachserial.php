...
class plgHikaserialAttachserial extends hikaserialPlugin {
	...
	public function onBeforeSerialMailSend(&$mail, &$mailer, &$serials, $order) {
		...
		foreach($plugins as $plugin) {
			$params = unserialize($plugin->plugin_params);
			...
			foreach($serials as $k => $serial) {
				...
				if(!empty($ret)) {
					...
					if(!isset($this->params)) {
						...
						$format = @$this->params['image_name_format'];
						$virtual_file = true;
						if(isset($this->params['virtual_file']))
							$virtual_file = $this->params['virtual_file'];
						$use_mime = true;
						if(isset($this->params['use_mime']))
							$use_mime = $this->params['use_mime'];
					} else {
	...
	private function displayTextBox($image, $size, $x, $y, $w, $h, $color, $font, $text) {
		...
		$lines = explode("\n", $text);
		$spaceWidth = imagettfbbox($size, 0, $font, ' ');
		$spaceWidth = $spaceWidth[2] - $spaceWidth[0];
		$dx = $x; $dy = $y;
		$mw = $x + $w;
	...
	private function generateImageQrCode($img, $d) {
		$level = 'l';
		$pixelPerPoint = 3;
		$outerFrame = 0;
		if(!empty($d['size']))
	...
	private function includeImage($img, $d) {
		...
		} else if(JFile::exists(HIKASHOP_MEDIA.'images'.DS.$filename)) {
