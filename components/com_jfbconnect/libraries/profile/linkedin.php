...
class JFBConnectProfileLinkedin extends JFBConnectProfile
{
    protected function setProviderFields()
    {
        $this->providerFields = array(
            ...
            /* Collections */
                ...
                'organizations-memberships.0' => 'Organization - 0 - Summary',
                'organizations-memberships.1' => 'Organization - 1 - Summary',
                'organizations-memberships.2' => 'Organization - 2 - Summary',
    ...
    public function getPermissionsForFields($fields)
    {
        ...
        foreach ($fields as $field)
        {
            if ($field == 'proposal-comments' ||
                    ...
                    strpos($field, 'organizations-memberships') !== false
            )
            {
                $scope[] = 'r_fullprofile';
            }
    ...
    */
    public function fetchProfile($userId, $fields)
    {
        ...
        if (in_array('organizations-memberships', $fields))
        {
            $fields[] = 'organizations-memberships:(id,name,position-held,start-date,end-date,description)';
            unset($fields[array_search('organizations-memberships', $fields)]);
        }
...
class JFBConnectProfileDataLinkedin extends JFBConnectProfileData
{
    ...
    //            $jlinkedLibrary->setTimestampOffset($offset);
                ...
                {
                    $app = JFactory::getApplication();
                    JFBCFactory::log('JFBConnect Notice: Server time not in sync with LinkedIn. Setting correction offset to: ' . $offset . 's', 'error');
                }
    ...
    public function get($path, $default = "")
    {
        ...
        switch ($valueParts[0])
        {
            ...
            case 'organizationsMemberships':
                return $this->getOrganizations($element, $index);
        }

        return $value;
    }

    private function getOrganizations($element, $index)
    {
        if (!property_exists($element->values, $index))
            return null;

        $organization = $element->values->$index;
        $dateString = $this->getDateRange($organization);

        $newValue = $organization->positionHeld . ' at ' . $organization->name;
        $newValue .= ': ' . $dateString;
        if ($organization->description)
            $newValue .= '. ' . $organization->description;
