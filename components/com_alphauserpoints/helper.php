	...
	{		
		...
		for ($i=0, $n=count( $ranks ); $i < $n; $i++)
		{
			...
			if ( $rank->ruleid>0 ) {
				...
				$query = "SELECT SUM(points) FROM #__alpha_userpoints_details " .
						 "WHERE rule='".$rank->ruleid."' AND approved='1' AND referreid='$referrerid' AND (`expire_date`>'$now' OR `expire_date`='0000-00-00 00:00:00')";
				$db->setQuery( $query );
		...
		for ($i=0, $n=count( $medals ); $i < $n; $i++)
		{
			...
			if ( $medal->ruleid>0 ) {
				// recalculate
				$query = "SELECT SUM(points) FROM #__alpha_userpoints_details " .
						 "WHERE rule='".$medal->ruleid."' AND approved='1' AND referreid='$referrerid' AND (`expire_date`>'$now' OR `expire_date`='0000-00-00 00:00:00')";
				$db->setQuery( $query );
				$currentpoints = $db->loadResult();
			} else $currentpoints = $userinfo->points;
